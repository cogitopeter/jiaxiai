---
name: gzhuaxue
description: "高中化学题目分析器。当用户发送化学题目（通常是拍照图片）时使用此skill。为每道题生成包含7个分析维度的白色打印友好HTML文件（解题思路、解题过程、知识点拆解、思维方式、易错点、变式题、难度定位），同时维护一个汇总索引页面用于学习诊断。触发词：'做这道题'、'分析这道题'、'这题怎么做'、发送数学题目图片。"
---

# 高中化学题目分析器

为高中化学题目生成详细的7维分析HTML文件，并维护学习诊断索引。不区分教材版本——题目按通用高中化学知识体系归类，若不同版本对同一知识点有不同命名习惯，可在标签中并列收录。

## 工作流程

### Step 0: 学科判断（前置检查）

在做任何事之前，先判断输入是否为化学题目。

**判断标准：** 题目的核心知识点属于化学学科（含高中化学内容）。

**不是化学题的典型情况：**
- 与化学学科无关的其他学科题目
- 其他任何非化学学科

**若判断为非化学题：** 输出以下提示后**立即终止**，不执行后续任何步骤，不生成任何文件：

> 这道题不属于化学范畴，/gzhuaxue 只处理化学题目。
> 如果题目里有需要单独分析的部分，请把那部分单独描述后再发给我。
...
工作目录：`~/Desktop/jiaxiai/gzhuaxue/`

1. 检查 `images/` 子目录是否存在，不存在则 `mkdir -p ~/Desktop/jiaxiai/gzhuaxue/images`
...
**图片输入（常见）：**
1. 将用户提供的图片复制到 `~/Desktop/jiaxiai/gzhuaxue/images/problem_NNN.jpg`
...
- 格式：`大模块 > 中模块 > 具体知识点`
- 列出所有涉及知识点，包括隐含的
- 不区分教材版本，按通用高中化学知识体系分类；不同版本对同一知识点有不同名称时可并列收录

**主模块分类（用于索引统计）：**
原子结构与元素周期律、化学键、化学反应原理、电化学、有机化学、物质的量、元素化合物

**超纲题处理：** 若题目涉及高中化学课标以外的知识点（即大学化学内容（如量子化学、结构化学等）），在维度三中注明所涉超纲知识点，`modules` 填写实际模块名，索引条目加 `overgrade: true`。超纲题正常生成分析页，但不计入学习诊断统计图表。
...
参照 **PROBLEM_TEMPLATE** 的结构生成 `problem_NNN.html`，保存到 `~/Desktop/jiaxiai/gzhuaxue/`。
...
读取 `~/Desktop/jiaxiai/gzhuaxue/index.html`，在 `var problems = [...]` 数组末尾、`];` 之前追加：
...
```javascript
{
  ...
  diffLabel: '中档',
  modules: ['原子结构与元素周期律'],
  moduleDetails: ['原子结构与元素周期律 > ...'],
  thinking: ['数形结合', '分类讨论'],
  source: '作业'
  // 若为超纲题（大学化学），加：overgrade: true
}
```
...
<div class="section" id="难度">
  ...
  <div class="difficulty-box">
    ...
    <div class="diff-row"><span class="diff-label">考试定位</span><span class="diff-value">高考压轴题 / 第X题水平</span></div>
...
当 `~/Desktop/jiaxiai/gzhuaxue/index.html` 不存在时生成。`var problems = [];` 初始为空，每次分析题目后追加。
...
<div class="header">
  ...
  <div style="font-size:15px;color:rgba(226,232,240,0.65);margin-top:10px;font-weight:500;letter-spacing:2px">高中化学 · 学习诊断</div>
...
<div class="footer">高中化学 · 学习诊断 &middot; 由 /gzhuaxue 自动维护<div style="margin-top:8px"><a href="http://www.he321.com" style="color:inherit;text-decoration:none">嘉析AI</a>官网：<a href="http://www.he321.com" style="color:inherit;text-decoration:underline">www.he321.com</a></div></div>

<script>
var problems = [];

var moduleNames=['原子结构与元素周期律','化学键','化学反应原理','电化学','有机化学','物质的量','元素化合物'];
var moduleColors={'原子结构与元素周期律':'#38bdf8','化学键':'#fbbf24','化学反应原理':'#f472b6','电化学':'#34d399',
  '有机化学':'#a78bfa','物质的量':'#fb923c','元素化合物':'#e879f9'};
