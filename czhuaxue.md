---
name: czhuaxue
description: "初中化学题目分析器。当用户发送化学题目（通常是拍照图片）时使用此skill。为每道题生成包含7个分析维度的白色打印友好HTML文件（解题思路、解题过程、知识点拆解、思维方式、易错点、变式题、难度定位），同时维护一个汇总索引页面用于学习诊断。触发词：'做这道题'、'分析这道题'、'这题怎么做'、发送数学题目图片。"
---

# 初中化学题目分析器

为初中化学题目生成详细的7维分析HTML文件，并维护学习诊断索引。不区分教材版本——题目按通用初中化学知识体系归类，若不同版本对同一知识点有不同命名习惯，可在标签中并列收录。
...
工作目录：`~/Desktop/jiaxiai/czhuaxue/`

1. 检查 `images/` 子目录是否存在，不存在则 `mkdir -p ~/Desktop/jiaxiai/czhuaxue/images`
...
**图片输入（常见）：**
1. 将用户提供的图片复制到 `~/Desktop/jiaxiai/czhuaxue/images/problem_NNN.jpg`
...
- 格式：`大模块 > 中模块 > 具体知识点`
- 列出所有涉及知识点，包括隐含的
- 不区分教材版本，按通用初中化学知识体系分类；不同版本对同一知识点有不同名称时可并列收录

**主模块分类（用于索引统计）：**
物质构成、化学反应、元素化合物、酸碱盐、金属、化学计算、实验探究

**超纲题处理：** 若题目涉及初中化学课标以外的知识点（即高中化学内容（如电化学、有机化学合成、化学平衡常数等）），在维度三中注明所涉超纲知识点，`modules` 填写实际模块名，索引条目加 `overgrade: true`。超纲题正常生成分析页，但不计入学习诊断统计图表。
...
参照 **PROBLEM_TEMPLATE** 的结构生成 `problem_NNN.html`，保存到 `~/Desktop/jiaxiai/czhuaxue/`。
...
读取 `~/Desktop/jiaxiai/czhuaxue/index.html`，在 `var problems = [...]` 数组末尾、`];` 之前追加：
...
当 `~/Desktop/jiaxiai/czhuaxue/index.html` 不存在时生成。`var problems = [];` 初始为空，每次分析题目后追加。
...
<div class="header">
  ...
  <div style="font-size:15px;color:rgba(226,232,240,0.65);margin-top:10px;font-weight:500;letter-spacing:2px">初中化学 · 学习诊断</div>
...
<div class="footer">初中化学 · 学习诊断 &middot; 由 /czhuaxue 自动维护<div style="margin-top:8px"><a href="http://www.he321.com" style="color:inherit;text-decoration:none">嘉析AI</a>官网：<a href="http://www.he321.com" style="color:inherit;text-decoration:underline">www.he321.com</a></div></div>

<script>
var problems = [];

var moduleNames=['物质构成','化学反应','元素化合物','酸碱盐','金属','化学计算','实验探究'];
var moduleColors={'物质构成':'#38bdf8','化学反应':'#fbbf24','元素化合物':'#f472b6','酸碱盐':'#34d399',
  '金属':'#a78bfa','化学计算':'#fb923c','实验探究':'#e879f9'};
