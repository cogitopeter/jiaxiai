...
工作目录：`~/Desktop/jiaxiai/czshuxue/`

1. 检查 `images/` 子目录是否存在，不存在则 `mkdir -p ~/Desktop/jiaxiai/czshuxue/images`
...
**图片输入（常见）：**
1. 将用户提供的图片复制到 `~/Desktop/jiaxiai/czshuxue/images/problem_NNN.jpg`
...
参照 **PROBLEM_TEMPLATE** 的结构生成 `problem_NNN.html`，保存到 `~/Desktop/jiaxiai/czshuxue/`。
...
读取 `~/Desktop/jiaxiai/czshuxue/index.html`，在 `var problems = [...]` 数组末尾、`];` 之前追加：
...
当 `~/Desktop/jiaxiai/czshuxue/index.html` 不存在时生成。`var problems = [];` 初始为空，每次分析题目后追加。
