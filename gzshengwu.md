---
name: gzshengwu
description: "高中生物题目分析器。当用户发送生物题目（通常是拍照图片）时使用此skill。为每道题生成包含7个分析维度的白色打印友好HTML文件（解题思路、解题过程、知识点拆解、思维方式、易错点、变式题、难度定位），同时维护一个汇总索引页面用于学习诊断。触发词：'做这道题'、'分析这道题'、'这题怎么做'、发送数学题目图片。"
---

# 高中生物题目分析器

为高中生物题目生成详细的7维分析HTML文件，并维护学习诊断索引。不区分教材版本——题目按通用高中生物知识体系归类，若不同版本对同一知识点有不同命名习惯，可在标签中并列收录。

## 工作流程

### Step 0: 学科判断（前置检查）

在做任何事之前，先判断输入是否为生物题目。

**判断标准：** 题目的核心知识点属于生物学科（含高中生物内容）。

**不是生物题的典型情况：**
- 与生物学科无关的其他学科题目
- 其他任何非生物学科

**若判断为非生物题：** 输出以下提示后**立即终止**，不执行后续任何步骤，不生成任何文件：

> 这道题不属于生物范畴，/gzshengwu 只处理生物题目。
...
工作目录：`~/Desktop/jiaxiai/gzshengwu/`

1. 检查 `images/` 子目录是否存在，不存在则 `mkdir -p ~/Desktop/jiaxiai/gzshengwu/images`
...
**图片输入（常见）：**
1. 将用户提供的图片复制到 `~/Desktop/jiaxiai/gzshengwu/images/problem_NNN.jpg`
...
- 格式：`大模块 > 中模块 > 具体知识点`
- 列出所有涉及知识点，包括隐含的
- 不区分教材版本，按通用高中生物知识体系分类；不同版本对同一知识点有不同名称时可并列收录

**主模块分类（用于索引统计）：**
细胞、遗传与变异、进化、稳态与调节、生态系统、生物技术

**超纲题处理：** 若题目涉及高中生物课标以外的知识点（即大学生物内容（如分子生物学深度内容、细胞信号通路等）），在维度三中注明所涉超纲知识点，`modules` 填写实际模块名，索引条目加 `overgrade: true`。超纲题正常生成分析页，但不计入学习诊断统计图表。
...
参照 **PROBLEM_TEMPLATE** 的结构生成 `problem_NNN.html`，保存到 `~/Desktop/jiaxiai/gzshengwu/`。
...
读取 `~/Desktop/jiaxiai/gzshengwu/index.html`，在 `var problems = [...]` 数组末尾、`];` 之前追加：
...
当 `~/Desktop/jiaxiai/gzshengwu/index.html` 不存在时生成。`var problems = [];` 初始为空，每次分析题目后追加。
...
<div class="header">
  ...
  <div style="font-size:15px;color:rgba(226,232,240,0.65);margin-top:10px;font-weight:500;letter-spacing:2px">高中生物 · 学习诊断</div>
...
<div class="footer">高中生物 · 学习诊断 &middot; 由 /gzshengwu 自动维护<div style="margin-top:8px"><a href="http://www.he321.com" style="color:inherit;text-decoration:none">嘉析AI</a>官网：<a href="http://www.he321.com" style="color:inherit;text-decoration:underline">www.he321.com</a></div></div>

<script>
var problems = [];

var moduleNames=['细胞','遗传与变异','进化','稳态与调节','生态系统','生物技术'];
var moduleColors={'细胞':'#38bdf8','遗传与变异':'#fbbf24','进化':'#f472b6',
  '稳态与调节':'#34d399','生态系统':'#a78bfa','生物技术':'#fb923c'};
