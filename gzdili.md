---
name: gzdili
description: "高中地理题目分析器。当用户发送地理题目（通常是拍照图片）时使用此skill。为每道题生成包含7个分析维度的白色打印友好HTML文件（解题思路、解题过程、知识点拆解、思维方式、易错点、变式题、难度定位），同时维护一个汇总索引页面用于学习诊断。触发词：'做这道题'、'分析这道题'、'这题怎么做'、发送数学题目图片。"
---

# 高中地理题目分析器

为高中地理题目生成详细的7维分析HTML文件，并维护学习诊断索引。不区分教材版本——题目按通用高中地理知识体系归类，若不同版本对同一知识点有不同命名习惯，可在标签中并列收录。

## 工作流程

### Step 0: 学科判断（前置检查）

在做任何事之前，先判断输入是否为地理题目。

**判断标准：** 题目的核心知识点属于地理学科（含高中地理内容）。

**不是地理题的典型情况：**
- 与地理学科无关的其他学科题目
- 其他任何非地理学科

**若判断为非地理题：** 输出以下提示后**立即终止**，不执行后续任何步骤，不生成任何文件：

> 这道题不属于地理范畴，/gzdili 只处理地理题目。
...
工作目录：`~/Desktop/jiaxiai/gzdili/`

1. 检查 `images/` 子目录是否存在，不存在则 `mkdir -p ~/Desktop/jiaxiai/gzdili/images`
...
**图片输入（常见）：**
1. 将用户提供的图片复制到 `~/Desktop/jiaxiai/gzdili/images/problem_NNN.jpg`
...
- 格式：`大模块 > 中模块 > 具体知识点`
- 列出所有涉及知识点，包括隐含的
- 不区分教材版本，按通用高中地理知识体系分类；不同版本对同一知识点有不同名称时可并列收录

**主模块分类（用于索引统计）：**
地球运动、大气、水体、地貌、人口与城市、产业与发展、区域地理、地图与统计

**超纲题处理：** 若题目涉及高中地理课标以外的知识点（即大学地理内容（如区位论深度应用、GIS专业技术等）），在维度三中注明所涉超纲知识点，`modules` 填写实际模块名，索引条目加 `overgrade: true`。超纲题正常生成分析页，但不计入学习诊断统计图表。
...
参照 **PROBLEM_TEMPLATE** 的结构生成 `problem_NNN.html`，保存到 `~/Desktop/jiaxiai/gzdili/`。
...
读取 `~/Desktop/jiaxiai/gzdili/index.html`，在 `var problems = [...]` 数组末尾、`];` 之前追加：
...
当 `~/Desktop/jiaxiai/gzdili/index.html` 不存在时生成。`var problems = [];` 初始为空，每次分析题目后追加。
...
<div class="header">
  ...
  <div style="font-size:15px;color:rgba(226,232,240,0.65);margin-top:10px;font-weight:500;letter-spacing:2px">高中地理 · 学习诊断</div>
...
<div class="footer">高中地理 · 学习诊断 &middot; 由 /gzdili 自动维护<div style="margin-top:8px"><a href="http://www.he321.com" style="color:inherit;text-decoration:none">嘉析AI</a>官网：<a href="http://www.he321.com" style="color:inherit;text-decoration:underline">www.he321.com</a></div></div>

<script>
var problems = [];

var moduleNames=['地球运动','大气','水体','地貌','人口与城市','产业与发展','区域地理','地图与统计'];
var moduleColors={'地球运动':'#38bdf8','大气':'#fbbf24','水体':'#f472b6','地貌':'#34d399',
  '人口与城市':'#a78bfa','产业与发展':'#fb923c','区域地理':'#e879f9','地图与统计':'#22d3ee'};
