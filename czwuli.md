...
1. 检查 `images/` 子目录是否存在，不存在则 `mkdir -p ~/Desktop/czwuli/images`
...
**图片输入（常见）：**
1. 将用户提供的图片复制到 `~/Desktop/czwuli/images/problem_NNN.jpg`
...
参照 **PROBLEM_TEMPLATE** 的结构生成 `problem_NNN.html`，保存到 `~/Desktop/czwuli/`。
...
读取 `~/Desktop/czwuli/index.html`，在 `var problems = [...]` 数组末尾、`];` 之前追加：
...
当 `~/Desktop/czwuli/index.html` 不存在时生成。`var problems = [];` 初始为空，每次分析题目后追加。
